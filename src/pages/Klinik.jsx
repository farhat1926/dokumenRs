import { useState } from "react";
import { komites } from "../data/komites";

export default function Klinik() {
  const [selectedUnit, setSelectedUnit] = useState(null);
  const [selectedKategori, setSelectedKategori] = useState(null);
  const [password, setPassword] = useState("");
  const [step, setStep] = useState(null); // "kategori" | "password"

  // const komites = [
  //   {
  //     nama: "Komite Medis",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1WYJ0l6CCRYs_DGxkanIe2cfn4suZC2jZ?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1mT8EUKmBetXy1bA8BIUJTgrNJJhDBhBA?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1lzVEFyKfm7TSAT-d-HENQYAKvwPiFrQB?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1Rz9UxEXb20UwGiByxlYkbXmTmtmGghFY?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Pedoman dan Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1UosUp1P1ShzrhWG-wL9QmHJgL1qjYrmx?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Monitoring dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1K5LhYnExn5cmKLitDtLAegFdEaKhpHEP?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Bukti Implementasi",
  //         drive: "https://drive.google.com/drive/folders/1s9ZRFAdItrJS-zHO7oS4gVf_iInepdVD?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Mutu",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1zg8xtIszwaGxoWHIEwut3RhRneBWB9Zb?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1zPhY3c5TN-GREGbPk-QKmIE-44mIB9C8?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1RvEoGcoL-zdokbCg3ZTM28I5tucwz5_q?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1ItF9fjW0T9vCpOn1E_fRjCrD_n4dbiJQ?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Pedoman dan Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1vjLbrpMiuvAdCjzM8lnzmmqFs-NMDeG2?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Monitoring dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1RGet-0_fYpuGZaf0ddk_p8jIwC8BNzBb?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //       {
  //         nama: "Bukti Implementasi",
  //         drive: "https://drive.google.com/drive/folders/1Vq9c6rSxiWRZtyZvAF2amXNo3eQS8l8U?usp=drive_link",
  //         password: "komitemedis",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Pencegahan & Pengendalian Infeksi",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1dqpQhd5QHeuLkMBfRCS3eynNQRrlROMe?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1ZsXh38fbT53zYZJKzAB93UOoZrlBVhqY?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/13K_xy5NfeQlQ0j4WxQ-mEz6vZSaRttFH?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Pedoman dan Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1xDQVO1gb2aMLGtOrRe30CyaYpCiU-KGP?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Monitoring dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1mjOp0gKtmmdfwKulkkcwsfqhjZQYQQYJ?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Bukti Implementasi",
  //         drive: "https://drive.google.com/drive/folders/11fdex99uNbMMez0tHhkKlv8PZ3LV2iiq?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Farmasi & Terapi",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1fNU3y59T97iOS8F0AnuXu67cIcRrDHWR?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/15Dx0y-65OHSCEez8_OIBVftDlssDMTgr?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1WR_abBUDUH8913PQbaqOvgOHmylXwR0u?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1_j4K8r_ke0svLKSuBZEjjT-auh4awQMz?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //       {
  //         nama: "Monitoring dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/10Jis-D2vg4kpaA5AYpcg9nLK3ULMXQes?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //       {
  //         nama: "Bukti Implementasi",
  //         drive: "https://drive.google.com/drive/folders/17S2mGtuYTDhaXrjqD2FD1i0EPuus0yts?usp=drive_link",
  //         password: "komitefarmasi",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Tenaga Kesehatan Lain",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/10lHWiNLlwDdIrIHjXWN61jU3QQXd4rwN?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1k1AdR5CBNdWv7Td4KyJ-taJecnBREi0U?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1tf5g6S--vwt15UZV67sIhU5XFBBXxjuY?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1MHYC73DdNAlQiHQGv9fu51_5HwSKda3T?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1aQWqmMOfhOjVw9fh9CjxAtcMQG-Df_F5?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //       {
  //         nama: "Bukti Implementasi",
  //         drive: "https://drive.google.com/drive/folders/17S2mGtuYTDhaXrjqD2FD1i0EPuus0yts?usp=drive_link",
  //         password: "komitetenaga",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Pencegahan & Pengendalian Infeksi",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1dqpQhd5QHeuLkMBfRCS3eynNQRrlROMe?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1xDQVO1gb2aMLGtOrRe30CyaYpCiU-KGP?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1ZsXh38fbT53zYZJKzAB93UOoZrlBVhqY?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/13K_xy5NfeQlQ0j4WxQ-mEz6vZSaRttFH?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1mjOp0gKtmmdfwKulkkcwsfqhjZQYQQYJ?usp=drive_link",
  //         password: "komitepencegahan",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Etik dan Hukum",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1TnexyTXYW8XvuEl_k6mjS-iUXpRTWiro?usp=drive_link",
  //         password: "komiteetik",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1myiSrDZ9KXxF9g2l24ppyeivmnbZqd8d?usp=drive_link",
  //         password: "komiteetik",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1yZ_soki_Z3mpkYJPe6zgXOFU1P5R_Xi0?usp=drive_link",
  //         password: "komiteetik",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1A8va-NmUBcQaY5pbojOjk9s1MMgH8L7W?usp=drive_link",
  //         password: "komiteetik",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/16EdsxhyO8zfKo9KpqHNDs3khMSjr_icA?usp=drive_link",
  //         password: "komiteetik",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Keselamatan & Kesehatan Kerja RS",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1e0onNoWoieH2VoBFgGUDMTOhBr5HI1cB?usp=drive_link",
  //         password: "komitekeselamatan",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1aI_eI8cNcNpbyDIJwETsEf2I172mz4_N?usp=drive_link",
  //         password: "komitekeselamatan",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1kWJoQk9GsbnYHfRRXMdv2wED9ES85IJz?usp=drive_link",
  //         password: "komitekeselamatan",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1sncLe0AfD4tFL_utlTHmiLGd2AXScR1N?usp=drive_link",
  //         password: "komitekeselamatan",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/13jTJbuDXK6rqNSCxpYyilPXGikLAVKY3?usp=drive_link",
  //         password: "komitekeselamatan",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Program Pengendalian Resistensi Antimikroba",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1__jmoe9gBOw4nJv25YHwyMokg4ZKY9ed?usp=drive_link",
  //         password: "komiteresistensi",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1a6A8F9aquDHxK3W7DXzXX_pgQmT1Qe1D?usp=drive_link",
  //         password: "komiteresistensi",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/122fazNFYAyUWK44uGxjDjIb3nCCu5PPe?usp=drive_link",
  //         password: "komiteresistensi",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/1WWn5o2Pun7s3PodOb2Ml1PGRPVA3r_oM?usp=drive_link",
  //         password: "komiteresistensi",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1_0sThDIspA-kazMbotlMxNah0-QnW1Q2?usp=drive_link",
  //         password: "komiteresistensi",
  //       },
  //     ],
  //   },
  //   {
  //     nama: "Komite Perawatan",
  //     kategori: [
  //       {
  //         nama: "SK & Organisasi",
  //         drive: "https://drive.google.com/drive/folders/1dCj0tZYLKx0V_vmKBWxgFXmg1g62kJvn?usp=drive_link",
  //         password: "komiteperawatan",
  //       },
  //       {
  //         nama: "Pedoman & Regulasi",
  //         drive: "https://drive.google.com/drive/folders/1VF3-BwxuVmaAAkBQvegdQleqUGL0vxu6?usp=drive_link",
  //         password: "komiteperawatan",
  //       },
  //       {
  //         nama: "Rapat & Evaluasi",
  //         drive: "https://drive.google.com/drive/folders/1EAA2dOK3bWNU1DnNbMs1IreZljvyePiz?usp=drive_link",
  //         password: "komiteperawatan",
  //       },
  //       {
  //         nama: "Program Kerja",
  //         drive: "https://drive.google.com/drive/folders/19QJUzYeyiO2dVtSwpMIh0VHi5u3hz4Ns?usp=drive_link",
  //         password: "komiteperawatan",
  //       },
  //       {
  //         nama: "Indikator Mutu dan Laporan",
  //         drive: "https://drive.google.com/drive/folders/1kn8q8PQzXoLATTM97dj3F1ybBkn-TxOL?usp=drive_link",
  //         password: "komiteperawatan",
  //       },
  //     ],
  //   },

  // ];

  const openUnit = (unit) => {
    setSelectedUnit(unit);
    setStep("kategori");
  };

  const openKategori = (kat) => {
    setSelectedKategori(kat);
    setStep("password");
    setPassword("");
  };

  const handleSubmit = () => {
    if (password === selectedKategori.password) {
      window.open(selectedKategori.drive, "_blank");
      resetAll();
    } else {
      alert("Password salah!");
    }
  };

  const resetAll = () => {
    setSelectedUnit(null);
    setSelectedKategori(null);
    setStep(null);
    setPassword("");
  };

  return (
    <div className="p-4 md:p-6">
      <h2 className="text-2xl font-bold mb-6">
        Dokumen Internal Rumah Sakit
      </h2>

      {/* LIST KOMITE */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {komites.map((unit, index) => (
          <button
            key={index}
            onClick={() => openUnit(unit)}
            className="bg-white shadow-md rounded-xl p-4 hover:shadow-lg hover:bg-blue-50 transition text-left"
          >
            <h3 className="text-lg font-semibold text-blue-900">
              {unit.nama}
            </h3>
            <p className="text-sm text-gray-500 mt-2">
              Klik untuk lihat dokumen
            </p>
          </button>
        ))}
      </div>

      {/* MODAL PILIH KATEGORI */}
      {step === "kategori" && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-80 md:w-96">
            <h3 className="text-xl font-bold mb-4">
              {selectedUnit?.nama}
            </h3>

            <div className="flex flex-col gap-3">
              {selectedUnit?.kategori.map((kat, i) => (
                <button
                  key={i}
                  onClick={() => openKategori(kat)}
                  className="p-3 border rounded-lg hover:bg-blue-50 text-left"
                >
                  📁 {kat.nama}
                </button>
              ))}
            </div>

            <button
              onClick={resetAll}
              className="mt-4 text-sm text-gray-500"
            >
              Tutup
            </button>
          </div>
        </div>
      )}

      {/* MODAL PASSWORD */}
      {step === "password" && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-80 md:w-96">
            <h3 className="text-xl font-bold mb-4">
              {selectedKategori?.nama}
            </h3>

            <input
              type="password"
              placeholder="Masukkan password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full border rounded-lg p-3 mb-4"
            />

            <div className="flex justify-end gap-3">
              <button
                onClick={() => setStep("kategori")}
                className="px-4 py-2 border rounded-lg"
              >
                Kembali
              </button>

              <button
                onClick={handleSubmit}
                className="px-4 py-2 bg-blue-900 text-white rounded-lg"
              >
                Masuk
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}